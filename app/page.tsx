...
export default function Home() {
  ...
  return <main className="app-shell" data-language={language}>
    <aside>
      ...
      <div className="sidebar-tools"><button className="language-toggle" onClick={()=>setLanguage(v=>v==="zh"?"en":"zh")}><b>{en?"中":"EN"}</b><span>{en?"中文":"English"}</span></button><button className="theme-toggle" onClick={()=>setDark(v=>!v)} aria-label="切换深色模式"><span>{dark?"☀":"◐"}</span>{dark?(en?"Light":"浅色模式"):(en?"Dark":"深色模式")}</button></div>
