...
export default function Home() {
  ...
  const dashboardNavLabel=en?"Dashboard":"工作台";
  const title=view==="dashboard"?(en?"Dashboard":"申请工作台"):view==="favorites"?(en?"Favorites":"收藏"):view==="schoolList"?(en?"My School List":"我的选校"):view==="mine"?t.mine:t.programs;
  ...
  return <main className="app-shell" data-language={language}>
    ...
    <aside className="sidebar" style={{position:"fixed"}}>
      ...
      <nav className="primary-nav">
        <button className={view==="dashboard"?"active":""} onClick={()=>setView("dashboard")}><span>⌂</span> {dashboardNavLabel}</button>
    ...
    <section className={`page page-${view}`} style={view==="dashboard"?{paddingTop:32}:undefined}>
      ...
      {view==="dashboard" && <section className="dashboard-view">
        <section className="command-overview"><div className="command-intro"><span>APPLICATION WORKSPACE</span><h2>{en?"2027 Fall Application Workspace":"2027 Fall 申请工作台"}</h2><p>{en?"Keep saved programs, material progress and reminders in one focused workspace.":"在一个清晰的工作台中查看收藏项目、材料进度和近期提醒。"}</p><div className="command-progress"><i style={{width:`${materialCompleted/Object.keys(materials).length*100}%`}}/><small>{en?`${materialCompleted} of ${Object.keys(materials).length} materials completed`:`申请材料已完成 ${materialCompleted} / ${Object.keys(materials).length}`}</small></div></div><div className="command-metrics"><button onClick={()=>setView("favorites")}><small>{en?"Favorites":"收藏"}</small><b>{favoriteIds.length}</b></button><button onClick={()=>setView("schoolList")}><small>{en?"School List":"选校名单"}</small><b>{schoolListItems.length}</b></button><button onClick={()=>setView("mine")}><small>{en?"Materials Done":"材料完成"}</small><b>{materialCompleted}/{Object.keys(materials).length}</b></button><button onClick={()=>setView("mine")}><small>{en?"Reminders":"日历提醒"}</small><b>{reminderNotes.length}</b></button></div></section>
    ...
    {compareOpen&&<div className="compare-overlay" onClick={()=>setCompareOpen(false)}><section className="compare-modal" onClick={e=>e.stopPropagation()}><button className="course-close" aria-label={en?"Close comparison":"关闭对比"} onClick={()=>setCompareOpen(false)}>×</button><p className="kicker">SCHOOL COMPARISON</p><h2>{en?"School Comparison":"学校对比"}</h2><div className="compare-table"><div className="compare-labels"><b>{en?"Program":"项目"}</b><span>{en?"National ranking":"全国大学排名"}</span><span>{en?"Application fee":"申请费"}</span><span>{en?"Tuition":"学费"}</span><span>{en?"Living costs":"生活费"}</span><span>{en?"Deadline":"截止日期"}</span><span>{en?"Location":"位置"}</span><span>{en?"Academic strengths":"研究优势"}</span><span>{en?"Official website":"官网"}</span></div>{compare.map(id=>{const p=PROGRAM_BY_ID.get(id);if(!p)return null;return <div className="compare-column" key={id}><b>{en?p.school:(SCHOOL_NAMES[p.school]||p.school)}</b><span>#{p.rank}</span><span>{APP_FEE_BY_REGION[programRegion(p)]}</span><span>{costFor(p.school).tuition}</span><span>{costFor(p.school).shared}</span><span>{dateLabel(p.deadline,language)}</span><span>{programLocation(p)}</span><span>{p.tracks.map(t=>t.name).join(" · ")}</span><a href={p.source} target="_blank" rel="noreferrer">{en?"Official site":"官网"} ↗</a></div>})}</div></section></div>}
